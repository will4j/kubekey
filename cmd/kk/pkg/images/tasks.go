...
func (p *PullImage) Execute(runtime connector.Runtime) error {
	if !p.KubeConf.Arg.SkipPullImages {
		...
		i.Images = []Image{
			...
			GetImage(runtime, p.KubeConf, "cilium-envoy"),
...
func GetImage(runtime connector.ModuleRuntime, kubeConf *common.KubeConf, name string) Image {
	...
	ImageList := map[string]Image{
		...
		"cilium-envoy":            {RepoAddr: kubeConf.Cluster.Registry.PrivateRegistry, Namespace: "cilium", Repo: "cilium-envoy", Tag: kubekeyv1alpha2.DefaultCiliumEnvoyVersion, Group: kubekeyv1alpha2.K8s, Enable: strings.EqualFold(kubeConf.Cluster.Network.Plugin, "cilium")},
	...
	imageOverrideCfg := kubeConf.Cluster.Registry.ImageOverrideCfg
	if len(imageOverrideCfg) > 0 {
		imageConfig, ok := imageOverrideCfg[name]
		if ok {
			if imageConfig.RepoAddr != "" {
				image.RepoAddr = imageConfig.RepoAddr
			}
			if imageConfig.Namespace != "" {
				image.NamespaceOverride = imageConfig.Namespace
			}
			if imageConfig.Repo != "" {
				image.Repo = imageConfig.Repo
			}
			if imageConfig.Tag != "" {
